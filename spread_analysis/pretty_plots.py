...
def list_available_csvs():
    ...
    for pattern in patterns:
        # Manual glob since we don't have glob
        files = [f for f in os.listdir('.') if f.startswith(pattern.replace('*', '').replace('.csv', '')) and f.endswith('.csv')]
        if files:
            print(f"\n{pattern}:")
            for f in files:
                print(f"  - {f}")
...
def main():
    ...
    if args.csv_file:
        ...
        if 'similarity_vs_accuracy_data' in args.csv_file:
            plot_similarity_vs_accuracy_from_csv(args.csv_file, args.model_type)
        elif 'violin_plot_data' in args.csv_file:
            plot_violin_plots_from_csv(args.csv_file, args.model_type)
        elif 'source_distribution_comparison' in args.csv_file:
            plot_source_distribution_from_csv(args.csv_file, args.model_type)
        elif 'generated_vs_noisy_reference_data' in args.csv_file:
            plot_generated_vs_noisy_reference_from_csv(args.csv_file, args.model_type)
        else:
            print(f"Unknown CSV file type: {args.csv_file}")
        return
